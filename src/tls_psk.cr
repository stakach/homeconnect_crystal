...
module HomeconnectLocal
  ...
  module TLS12PSK
    ...
    @[Link("ssl")]
    lib LibSSLExtras
      alias SSL_CTX = LibSSL::SSLContext
      alias SSL = LibSSL::SSL

      alias PskClientCallback = (SSL, LibC::Char*, LibC::Char*, UInt32, UInt8*, UInt32 -> UInt32)
      fun SSL_CTX_set_psk_client_callback(ctx : SSL_CTX, cb : PskClientCallback) : Void
    end
    ...
    @@client_cb : LibSSLExtras::PskClientCallback = ->(
      ssl : LibSSLExtras::SSL,
      hint : LibC::Char*,
      identity_buf : LibC::Char*,
    ...
    private def self.set_tls12_only!(raw : LibSSLExtras::SSL_CTX)
    ...
    def self.build_client_context(psk64 : String, identity : String = "homeconnect", cipher : String = "PSK") : OpenSSL::SSL::Context::Client
      ...
      raw = ctx.to_unsafe
      set_tls12_only!(raw)
      LibSSLExtras.SSL_CTX_set_psk_client_callback(raw, @@client_cb)
