...
module HomeconnectLocal
  module Parser
    ...
    def self.parse_feature_mapping(xml : String) : FeatureMap
      doc = XML.parse(normalize_xml(xml))
      fm = FeatureMap.new
    ...
    def self.parse_device_description(device_xml : String, feature_xml : String) : DeviceDescription
      ...
      doc = XML.parse(normalize_xml(device_xml))
    ...
    # Device XML files commonly use a default xmlns. Crystal's XPath
    # lookups in this parser are namespace-unaware, so we strip only the
    # default namespace declarations to keep local tag names queryable.
    private def self.normalize_xml(xml : String) : String
      xml.gsub(/\sxmlns=(['"]).*?\1/, "")
    end
