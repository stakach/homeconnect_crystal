...
module HomeconnectLocal
  ...
  class Entity
    ...
    def value_raw=(value : JSON::Any)
      ...
      payload = JSON.parse([{"uid" => @uid, "value" => converted.raw}].to_json).as_a
  ...
  class Program < Entity
    ...
    def select
      payload = JSON.parse([{"program" => @uid, "options" => [] of Hash(String, JSON::Any)}].to_json).as_a
    ...
    def start(options : Hash(Int32, JSON::Any) = {} of Int32 => JSON::Any, override_options : Bool = false, entities_by_uid : Hash(Int32, Entity)? = nil)
      ...
      payload = JSON.parse([{"program" => @uid, "options" => opts}].to_json).as_a
